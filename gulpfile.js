...
gulp.task('run', ['copy-static'], function(callback) {
    ...
    webpackServerInstance.listen(8000, "localhost", function(err) {
            if(err) {
                throw new gutil.PluginError('webpack:dev-server', err);
            }

            gutil.log('[run]', 'Dynamic reload: http://localhost:8000/webpack-dev-server/index.html');
            gutil.log('[run]', 'Static version: http://localhost:8000/index.html');
            callback();
        }
    );
});
